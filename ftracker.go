...
func RunningSpentCalories(action int, weight, duration float64) float64 {
	// ваш код здесь
	rSpent := ((runningCaloriesMeanSpeedMultiplier * runningCaloriesMeanSpeedShift * meanSpeed(action, duration)) * weight / mInKm * duration * minInH)
	return rSpent
}
...
func WalkingSpentCalories(action int, duration, weight, height float64) float64 {
	// ваш код здесь
	wSpent := (walkingCaloriesWeightMultiplier*weight + ((math.Pow(meanSpeed(action, duration)*kmhInMsec, 2) / (height * cmInM)) * walkingSpeedHeightMultiplier * weight)) * duration * minInH
	return wSpent
}
...
func SwimmingSpentCalories(lengthPool, countPool int, duration, weight float64) float64 {
	// ваш код здесь
	swSpent := (swimmingMeanSpeed(lengthPool, countPool, duration) + swimmingCaloriesMeanSpeedShift) * swimmingCaloriesWeightMultiplier * weight * duration
	return swSpent
}
